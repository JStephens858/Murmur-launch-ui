...
**Decision:** `public/info/privacy_policy.html`, `terms_of_service.html` and `community_guidelines.html` are regenerated from the three "Final 2026" `.docx` files counsel supplied. Same URLs (the App Store listing and the iOS app link to them), same `?mode=dark|light` contract, new markup. The Word sources live in `legal-sources/` (not `public/`, which is served verbatim — the drafts carry yellow placeholders and law-firm document IDs that must not be public). `content_policy.html` had no new draft; its text is unchanged but it shares the new styling.
**How the pages are built:** one shared `info.css` on the palette from `app/globals.css` (`#f4f6f7`/`#232a33` light, `#14181d`/`#edf1f3` dark, `#de046c` primary, `#8a1e5c`/`#d98bb8` accent), Inter with a system fallback, 46rem measure, a collapsible table of contents on the two long documents, bordered callouts for the all-caps notices, and the DMCA form rendered as a printable form block. `info.js` runs in `<head>` and sets `data-theme` on `<html>` from, in order: `?mode=` (what the iOS webview passes), next-themes' `localStorage.theme` (so a site visitor in dark mode gets dark here), then `prefers-color-scheme`. It also appends the explicit `?mode=` to cross-links between the legal pages, but only when one was given — the old script appended `?mode=null` when there wasn't, and that literal string had made it into counsel's Word drafts as link targets.
**Text fidelity:** a word-level diff of each `.docx` against the rendered HTML shows only the corrections below plus heading case.
**Mechanical corrections applied (typos only):** "Murumur" → "Murmur"; "reserves the right remove" → "right to remove"; "Otherwise with us and other users" → "Otherwise interact with us…"; "dissemble" → "disassemble"; "please following" → "please follow"; "Expect as provided herein" → "Except…"; "Judicial Arbitration and Media Services" → "…Mediation Services"; "signed by the you" → "signed by you"; "athttp://" spacing; the Posting Policy's last item re-lettered (e) instead of restarting at (a); "Representations, and Limitations" comma dropped; a missing full stop.
**Content decisions made by Josh on 2026-09-08 after review of counsel's drafts:** the Terms are titled "Terms of Service" (the draft said "Terms of Use"; every other document and the footer already said Service); the Privacy Policy age threshold is 18, matching the Terms; the arbitration opt-out email is `contact@murmurmd.com` (the draft's `contact@murmurapp.com` domain does not exist); a "Contact Us" section with the Boise mailing address (14050 N Almond Ave, Boise, ID 83714) closes the Privacy Policy so its COPPA paragraph's "address at the end of this Policy" resolves; "; Call Recordings" is dropped from the contact-authorization heading (no such content); "Drug Enforcement Agency" → "Administration"; all four pages carry the Privacy Policy's effective date (2021-11-19) and a last-updated date of 2026-09-08. Left as drafted on purpose: the "Additional Terms" example list, "on the App or App", the notification-obligations sentence in Privacy §E, the mixed-grammar prohibited-activities list and the duplicate severability clause.
**Content Policy** was restyled onto the same `info.css`/`info.js` and linked from the Guidelines' header and intro; `info_light.css`/`info_dark.css` are deleted since nothing references them.
**Status:** Active

## 2026-08-19 — Reengagement emails restyled to the site's design system (dark), on table layout
**Decision:** `reengagementEmails/reengagementEmail.html` and `reengagementEmails/reengagementPreview.html` are rebuilt against the palette in `app/globals.css`, using **`.dark`**: `#14181D` ground, `#1A2027` cards with a `#30373E` hairline and 14px radius, `#EDF1F3` headings on `#9AA6B2` body text, `#DE046C` primary buttons running the dark gradient (full primary down to primary/70, `#A30C57`), and `#D98BB8` on the `#3A1F31` accent tint for group chips, the CTA panel and links. Inter with a system fallback stack. The purple/blue gradients, the `#dc3545` reject button and the old `#2d3748` footer are gone.
**Both themes build; dark is what ships.** `generate-preview.py` takes `dark|light` as its second argument and carries both palettes, so reverting the preview is a rebuild rather than a rewrite. The real email has no generator — if it ever needs to go back, rebuild the preview in light and match the hexes by hand.
**Why dark is baked in rather than switched.** Email clients have no `next-themes`; the only mechanism is a `prefers-color-scheme` media query, which Apple Mail honours, Gmail's web client ignores entirely, and the Gmail apps apply inconsistently — a two-theme email is therefore a light email that some clients darken badly on their own terms. One committed theme renders the same everywhere. `color-scheme: dark` and `supported-color-schemes: dark` are declared both in a meta tag and in `:root`, so clients that auto-invert leave an already-dark design alone.
**Colours the site expresses as alpha are flattened,** because email CSS has no equivalent of Tailwind's colour-mix. Dark card borders are `border-border/15`, so `#B0BBBF` at 15% over `#1A2027` is precomputed to `#30373E` — and to `#2B3035` over the page ground, where the hairline rules sit. Likewise the reject button: `destructive/30` over the card is `#391D23`, carrying `--destructive-foreground` `#FB2C36`.
**Every `{{handlebars}}` name is unchanged** — 361 in the preview, 12 in the real email — and `display:` toggles still sit on block-level wrappers, so `{{{groupNdisplay}}}` / `{{{postNDisplayValue}}}` keep working exactly as before. Verified by extracting the variable sets from both files and diffing against the originals, including brace depth (triple for the display values, double for everything else).
**Layout moved from flexbox to nested `<table role="presentation">` with inlined styles.** The old templates used `display:flex` and `gap`, neither of which Outlook's Word rendering engine supports; the `<style>` block is now progressive enhancement only (hover states and the ≤620px stacking media query).
**Two deliberate changes beyond styling:**
- The preview's "Accept all posts" button is repeated in the masthead, not only after twenty groups. Same `{{acceptAllPostsButtonUrl}}`, so no new template surface. The stray `margin-bottom: 1000px` on the old one is dropped.
- The real email's intro line is count-neutral ("Here's some of what you missed…") because `{{{post2DisplayValue}}}` can hide the second post, and the old copy promised "a couple recent posts" regardless.

**The preview template is generated, not hand-edited.** It is twenty identical group blocks by two posts, so `reengagementEmails/reengagementPreview.html` is a build artifact of `reengagementEmails/generate-preview.py` — editing the markup in twenty places by hand is how variable names drift. Regenerate with `python3 reengagementEmails/generate-preview.py reengagementEmails/reengagementPreview.html`.
**The masthead uses the site's own wordmark**, `https://murmurmd.com/web_logo_dark.png`, not the `murmur_email_header.png` that had been sitting in the S3 misc bucket. `public/web_logo_dark.png` is the dark-background lockup — pink glyph, **white** wordmark — which is what a `#14181D` ground wants; the S3 file was a pink wordmark on transparent, legible but not the brand's dark treatment. Both `web_logo_*.png` are already served from production (verified 200, byte sizes matching `public/`), so no new asset had to be published anywhere. `generate-preview.py` picks the lockup per theme.
**Stale comment worth correcting:** `components/logos/murmurmd.tsx` says the two logo files "are currently identical — light needs a dark-text variant from branding". They are not identical (differing md5, and the dark one visibly carries a white wordmark). That comment predates the current assets.
**Not done:** no test send through SendGrid, and no render check in Outlook/Gmail/Apple Mail — dark designs are exactly where those clients diverge, so a real send is worth doing before this ships. Verified in Chrome at 680px and at 390px, with hidden groups and a hidden post.
**Status:** Active
